...
edge<-function(folder_name){
    ...
    write.csv(edge_list, "modified_edge.csv", row.names=F) #출발동_도착동_출발행코_도착행코
...
if(edge_list[330,4]==gdong[450,2]){
    edge_list[330,4]<-as.numeric(substr(edge_list[330,4],1,5))
...
#엣지리스트의 경기, 인천 코드 자릿수는 5개이므로 nchar = 5인 행정 코드만 선택
S.gyunggi<-subset(edge_list, nchar(Start_cd)==5, select = Start_cd)
S.uniq<-unique(S.gyunggi$Start_cd)                                  #유니크한 코드 

#끝점
E.gyunggi<-subset(edge_list, nchar(End_cd)==5, select = End_cd)
E.uniq<-unique(E.gyunggi$End_cd)

#경기, 인천 시군구 개수 gyunggi.n: 38개 
gyunggi.n<-NROW(union(S.uniq,E.uniq))
...
colnames(empty.A)<-c(sdong[,3],gyunggi.sigungu)
rownames(empty.A)<-colnames(empty.A)
#colnames(empty.A) -- 아래 경기도 시군구명 정렬된거 확인 
View(empty.A)
