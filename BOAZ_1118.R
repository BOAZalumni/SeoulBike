...
CSV_to_SHP <- function(foldername, datacrs=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
{
  ...
  for (i in 1:count) {
    ...
    colnames(shp) <- c("latitude", "longitude", "elevation", "grade", "speed", "direction", "total_distance", 
                       "total_time_moving", "save_date", "month", "weekday", "hour", "dong", "holidays")
...
BOAZ <- function(foldername)
{
  ...
  holidays_list <- read.table('holidays_list.txt',sep = "\n")
  
  #
  # greentrack 파일을 파일 리스트에서 제거.
  b_files <- grepl("green", src_files)
  if(sum(b_files) != 0)
    src_files <- src_files[!b_files]
  
  # _l 파일을 파일 리스트에서 제거.
  b_files <- grepl("_l.kml", src_files)
  if(sum(b_files) != 0)
    src_files <- src_files[!b_files]
  #
  summary(src_files)
  
  # 중복 파일 제거 (이 위치에서 33472, 17995 번은 예외적인 이름.)
  splname <- strsplit(src_files, "_")
  dup_index <- duplicated(paste(sapply(splname, function(l) l[1]),sapply(splname, function(l) l[length(l)]),sep=""))
  src_files <- src_files[!dup_index]
  
  file_count <-length(src_files) # 1년치 데이터에서 남은 개수 54982
  ...
  # 에러 저장 csv생성.
  df_err <- data.frame(matrix(ncol=3))[-1,]; colnames(df_err) <- c("Number", "ID", "Error type")
  df_err[1,] <- c(1, "default", "default")
  write.csv(df_err, "error.csv", row.names = FALSE)
    ...
    tryCatch(
      {
        ...
        if(sum(des)>0){
          ...
          if((velocity >= 6000) & (velocity <= 30000))
          {
            ...
            # Max speed 거르기 위한 저장.
            maxspeed <- max(path.mat[,"speed"])
            
            # 변수개수 줄이기
            sel.col <- c("latitude", "longitude", "total_distance", "save_date")
      ...
      error=function(cond){
        df_err <- read.csv("error.csv", stringsAsFactors = FALSE)
        if(nrow(df_err) == 1){
          df_err$Number <- as.integer(df_err$Number)
          df_err$ID <- as.character(df_err$ID);df_err$Error.type <-as.character(df_err$Error.type)
          if(class(cond) == "List"){
            df_err[2,1]<-as.integer(i)
            df_err[2,2]<-as.character(src_files[i])
            df_err[2,3]<-as.character(cond[[1]])
          }else{
            df_err[2,1]<-as.integer(i)
            df_err[2,2]<-as.character(src_files[i])
            df_err[2,3]<-as.character(cond)
          }
          
        }else{
          
          row_insert <- nrow(df_err) + 1
          if(class(cond) == "List"){
            df_err[row_insert,1]<-as.integer(i)
            df_err[row_insert,2]<-as.character(src_files[i])
            df_err[row_insert,3]<-as.character(cond[[1]])
          }else{
            df_err[row_insert,1]<-as.integer(i)
            df_err[row_insert,2]<-as.character(src_files[i])
            df_err[row_insert,3]<-as.character(cond)
          }
...
# CSV읽어서 속력 - 거리 그래프 만들기.
foldername <- "2012-12-31"
path <- paste("work", foldername, sep="/")
files <- list.files(path)
filecnt <- length(files)

totdist <- numeric(filecnt)
tottime <- numeric(filecnt)
avgspeed <- numeric(filecnt)
for(i in 1:filecnt)
{
  df_track <- read.csv(paste(path, files[i], sep="/"))
  
  totdist[i] <- df_track[nrow(df_track), 7]
  tottime[i] <- (df_track[nrow(df_track),9] - df_track[1,9]) / 1000
  
  avgspeed[i] <- totdist[i] / tottime[i]
...
hist(totdist)
hist(tottime)
plot(avgspeed, totdist)
plot(avgspeed, tottime)
plot(tottime, totdist)


# 전체 kml에 대해 속력 거리 시간분포.
src_files <- list.files(foldername)
...
totdist <- numeric(file_count)
tottime <- numeric(file_count)
avgspeed <- numeric(file_count)
maxspeed <- numeric(file_count)
...
for( i in 1:file_count){
  ...
  totdist[i] <- xmlAttrs(root[[nrows+1]][["rr_location_total_distance"]])
  tottime[i] <- (xmlAttrs(root[[2]][[ncols]]) - xmlAttrs(root[[nrows+1]][[ncols]])) / 1000
...
GetFilelist <- function(foldername)
{
  src_files <- list.files(foldername)
  # greentrack 파일을 파일 리스트에서 제거.
  b_files <- grepl("green", src_files)
...
foldername <- "2012-12-31"
src_files <- GetFilelist(foldername)
file_count <-length(src_files)

v_name <- sapply(strsplit(src_files, "_"), function(l) l[1])
v_uname <- unique(v_name) # 확인용

df_usercnt <- rle(v_name)
df_usercnt <- data.frame(UserID = df_usercnt$values, Count = df_usercnt$lengths)
